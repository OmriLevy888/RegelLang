...
public:
  TextSourceStream(const std::string &source, size_t fileIndex = 0)
      : m_sourceText(source), m_fileIndex(fileIndex), m_pos(0),
        m_end(source.length()) {}
  ...
  inline bool eof() const override { return m_pos > m_end; }
  inline bool isOpen() const override { return m_pos < m_end; }
